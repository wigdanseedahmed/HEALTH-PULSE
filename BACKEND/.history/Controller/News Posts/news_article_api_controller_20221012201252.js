...
const getNewsArticleByPublisher = ( (req, res) => {
  NewsArticlesModel.findOne({ publishedBy: req.params.publishedBy }, (err, result) => {
    if (err) return res.status(500).json({ msg: err });
    return res.json({
      data: result,
...
module.exports = {
  ...
  getNewsArticleByPublisher: getNewsArticleByPublisher,
