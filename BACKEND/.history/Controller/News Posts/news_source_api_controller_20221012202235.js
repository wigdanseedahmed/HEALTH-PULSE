...
const getNewsCategoryByAuthor = ( (req, res) => {
  NewsCategorysModel.findOne({ author: req.params.author }, (err, result) => {
    if (err) return res.status(500).json({ msg: err });
    return res.json({
      data: result,
...
const getNewsCategoryByPublisher = ( (req, res) => {
  NewsCategorysModel.findOne({ publishedBy: req.params.publishedBy }, (err, result) => {
    if (err) return res.status(500).json({ msg: err });
    return res.json({
      data: result,
      publishedBy: req.params.publishedBy,
    });
  });
});
...
module.exports = {
  ...
  getNewsCategoryByAuthor: getNewsCategoryByAuthor,
  getNewsCategoryByPublisher: getNewsCategoryByPublisher,
